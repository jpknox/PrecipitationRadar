	...
	this.buildImage = function() {
		...
		for (y = 0; y < rows; y++) {
			for (x = 0; x < columns; x++) {
				...
				mapTile.onload = wrapDrawImage(context, mapTile, x, y);
		...
		console.log("Canvas data Url: " + canvas.toDataURL("image/png"))
		return canvas.toDataURL("image/png");
	}

	var wrapDrawImage = function(context, mapTile, x, y) {
		return function() {
...
function metOfficeUrl() {
	...
	this.tileRowValue = '0';
	this.tileColValue = '0';
	this.dimRunValue = '2017-11-24T13:30:00Z';
